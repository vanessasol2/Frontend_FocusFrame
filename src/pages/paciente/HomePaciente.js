...
const HomePaciente = () => {
  ...
  const resumenData = [
    { title: "Total Citas", value: 15, color: "text-gray-700", bg: "bg-[#f3f0ff]", icon: totalCita },
    { title: "Citas Pendientes", value: 4, color: "text-yellow-600", bg: "bg-yellow-100", icon: citasPendientes },
    { title: "Citas Canceladas", value: 1, color: "text-red-600", bg: "bg-red-100", icon: cancelarCita },
  ];

  return (
    <MainLayout>
      {/* Sección de Resumen */}
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-10 px-6">
        {resumenData.map((item, index) => (
          <div
            key={index}
            className={`${item.bg} p-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-between`}
          >
            <div>
              <h4 className="font-semibold text-gray-600 text-lg">{item.title}</h4>
              <span className={`text-5xl font-bold ${item.color}`}>{item.value}</span>
            </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10 px-6">
        {/* Citas Recientes */}
        <div>
          <h1 className="text-2xl font-bold text-gray-700 mb-6">Citas Recientes</h1>
        ...
        <div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-inner overflow-y-auto max-h-96">
            {Array.from({ length: 5 }, (_, index) => (
              <div key={index} className="bg-[#f3f0ff] p-4 rounded-lg shadow mb-4 last:mb-0">
                <h3 className="text-lg font-semibold text-gray-800 mb-2">Nota {index + 1}</h3>
                <p className="text-sm text-gray-600">Esta es la nota número {index + 1}. Muy bien</p>
              </div>
            ))}
          </div>
        </div>
